...
import { AssetUtil, ChainController, ConnectionController, ConnectorController, ModalController, RouterController, SnackController } from '@reown/appkit-controllers';
...
vi.mock('../../src/utils/AssetUtil.js', () => ({
    ...
    formatAmount: vi.fn((val) => String(val)),
...
describe('W3mPayView', () => {
    ...
    beforeEach(() => {
        ...
        vi.mocked(formatAmount).mockImplementation((val) => String(val));
    });
    ...
    test('should initialize and set selectedExchange to undefined', async () => {
        await fixture(html `<w3m-pay-view></w3m-pay-view>`);
    ...
    test('should subscribe to active chain and update caipAddress', async () => {
        const element = await fixture(html `<w3m-pay-view></w3m-pay-view>`);
        await elementUpdated(element);
        expect(ChainController.subscribeChainProp).toHaveBeenCalledWith('accountState', expect.any(Function), 'eip155');
    });
    ...
    test('should subscribe to ConnectorController activeConnectorIds changes', async () => {
        const element = await fixture(html `<w3m-pay-view></w3m-pay-view>`);
        await elementUpdated(element);
        expect(ConnectorController.subscribeKey).toHaveBeenCalledWith('activeConnectorIds', expect.any(Function));
    });
    ...
    test('should render list items with correct properties', async () => {
        ...
        const firstExchangeOption = element.shadowRoot?.querySelector('[data-testid="exchange-option-coinbase"]');
