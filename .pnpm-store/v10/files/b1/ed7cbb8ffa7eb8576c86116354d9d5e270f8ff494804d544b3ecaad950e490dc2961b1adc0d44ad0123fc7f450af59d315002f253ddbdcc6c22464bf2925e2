...
describe('ConnectorController', () => {
    ...
    it('should find connector by active namespace only', () => {
        ...
        ChainController.state.activeChain = ConstantsUtil.CHAIN.EVM;
        expect(ConnectorController.getConnector({ id: EVM_EXPLORER_ID, namespace: 'eip155' })).toEqual(evmConnector);
        expect(ConnectorController.getConnector({ id: SOLANA_EXPLORER_ID, namespace: 'eip155' })).toBeUndefined();
    ...
    it('should return the correct connector on getConnector', () => {
        ...
        expect(ConnectorController.getConnector({ id: 'walletConnectId', namespace: 'eip155' })).toStrictEqual(walletConnectConnector);
        expect(ConnectorController.getConnector({ id: metamaskConnector.id, namespace: 'eip155' })).toStrictEqual(metamaskConnector);
        expect(ConnectorController.getConnector({ id: zerionConnector.id, namespace: 'eip155' })).toStrictEqual(zerionConnector);
    ...
    it('should call mobile wallet util when selecting wallet is Phantom ', () => {
        ...
        const handleMobileDeeplinkRedirectSpy = vi.spyOn(MobileWalletUtil, 'handleMobileDeeplinkRedirect');
        vi.spyOn(ConnectorController, 'getConnector').mockReturnValue(mockConnector);
        vi.spyOn(RouterController, 'push');
        ...
        expect(handleMobileDeeplinkRedirectSpy).toHaveBeenCalledWith(mockConnector.id, ConstantsUtil.CHAIN.EVM, { isCoinbaseDisabled: false });
    });
    it('should redirect to Coinbase deeplink when selecting Coinbase wallet on Solana', () => {
        ...
        expect(window.location.href).toBe(expectedUrl);
        expect(handleMobileDeeplinkRedirectSpy).toHaveBeenCalledWith(mockConnector.id, ConstantsUtil.CHAIN.SOLANA, { isCoinbaseDisabled: false });
    });
    ...
    it('should not call redirect when selected wallet is Coinbase and active chain is not Solana or EVM', () => {
        ...
        expect(handleMobileDeeplinkRedirectSpy).toHaveBeenCalledWith(mockConnector.id, ConstantsUtil.CHAIN.BITCOIN, { isCoinbaseDisabled: false });
    });
