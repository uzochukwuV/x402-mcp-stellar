...
import {} from '../../src/controllers/ChainController';
...
describe('BalanceUtil', () => {
    ...
    describe('getMyTokensWithBalance', () => {
        ...
        it('should use BlockchainApi for Solana chain', async () => {
            ...
            const result = await BalanceUtil.getMyTokensWithBalance();
            expect(BlockchainApiController.getBalance).toHaveBeenCalledWith(mockSolanaAddress, mockSolanaNetwork.caipNetworkId, undefined);
        ...
        it('should use BlockchainApi if connector is not auth', async () => {
            ...
            ChainController.state.activeCaipNetwork = mockEthereumNetwork;
            vi.spyOn(ConnectorController, 'getConnectorId').mockReturnValue(ConstantsUtil.CONNECTOR_ID.INJECTED);
            vi.mocked(BlockchainApiController.getBalance).mockResolvedValue({ balances: mockBalances });
            const result = await BalanceUtil.getMyTokensWithBalance();
            expect(BlockchainApiController.getBalance).toHaveBeenCalledWith(mockEthereumAddress, mockEthereumNetwork.caipNetworkId, undefined);
            expect(result).toEqual(mockBalances);
        });
        ...
        it('should return cached balance from storage if it exists', async () => {
            ...
            const result = await BalanceUtil.getMyTokensWithBalance();
            expect(StorageUtil.getBalanceCacheForCaipAddress).toHaveBeenCalledWith(`${mockEthereumNetwork.caipNetworkId}:${mockEthereumAddress}`);
    ...
    describe('fetchERC20Balance', () => {
        ...
        it('should handle network errors gracefully', async () => {
            ...
            await expect(BalanceUtil.fetchERC20Balance({
                caipAddress: mockCaipAddress,
                assetAddress: mockAssetAddress,
                caipNetwork: mockCaipNetwork
...
//# sourceMappingURL=BalanceUtil.test.js.map