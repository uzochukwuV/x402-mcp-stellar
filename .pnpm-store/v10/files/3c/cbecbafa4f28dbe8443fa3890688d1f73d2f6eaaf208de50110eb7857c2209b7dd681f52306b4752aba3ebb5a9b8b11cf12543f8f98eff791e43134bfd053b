...
import { formatCaip19Asset, getApiUrl, getBuyStatus, getExchanges, getPayUrl } from '../../src/utils/ExchangeUtil';
describe('ExchangeUtil', () => {
    ...
    describe('getApiUrl', () => {
        it('returns the correct WalletConnect JSON-RPC URL', () => {
            const url = getApiUrl();
            expect(url).toBe('https://rpc.walletconnect.org/v1/json-rpc?projectId=test-project-id&st=test-sdk-type&sv=test-sdk-version&source=fund-wallet');
        });
    });
    describe('JSON-RPC requests', () => {
        it('getExchanges sends request and returns result', async () => {
            ...
            expect(String(calledUrl)).toBe('https://rpc.walletconnect.org/v1/json-rpc?projectId=test-project-id&st=test-sdk-type&sv=test-sdk-version&source=fund-wallet');
    ...
    describe('formatCaip19Asset', () => {
        it('formats EVM native asset', () => {
            const formatted = formatCaip19Asset('eip155:1', 'native');
            expect(formatted).toBe('eip155:1/slip44:60');
        });
        it('formats EVM ERC20 token', () => {
            const formatted = formatCaip19Asset('eip155:1', '0xabc');
            expect(formatted).toBe('eip155:1/erc20:0xabc');
        });
        it('formats Solana native asset', () => {
            const formatted = formatCaip19Asset('solana:5eykt4UsFv8P8NJdTREpY1vzqKqZKvdp', 'native');
            expect(formatted).toBe('solana:5eykt4UsFv8P8NJdTREpY1vzqKqZKvdp/slip44:501');
        });
        it('formats Solana token', () => {
            const formatted = formatCaip19Asset('solana:5eykt4UsFv8P8NJdTREpY1vzqKqZKvdp', 'So11111111111111111111111111111111111111112');
            expect(formatted).toBe('solana:5eykt4UsFv8P8NJdTREpY1vzqKqZKvdp/token:So11111111111111111111111111111111111111112');
        });
        it('throws for unsupported chain namespace', () => {
            expect(() => formatCaip19Asset('cosmos:cosmoshub-4', 'native')).toThrow('Unsupported chain namespace for CAIP-19 formatting: cosmos');
        });
        it('formats BNB native asset on Binance Smart Chain with correct SLIP-44', () => {
            const formatted = formatCaip19Asset('eip155:56', 'native');
            expect(formatted).toBe('eip155:56/slip44:714');
        });
        it('formats BNB native asset on opBNB with correct SLIP-44', () => {
            const formatted = formatCaip19Asset('eip155:204', 'native');
            expect(formatted).toBe('eip155:204/slip44:714');
        });
        it('formats ERC20 token on Binance Smart Chain (not affected by override)', () => {
            const formatted = formatCaip19Asset('eip155:56', '0xabc');
            expect(formatted).toBe('eip155:56/erc20:0xabc');
        });
    });
});
//# sourceMappingURL=ExchangeUtil.test.js.map