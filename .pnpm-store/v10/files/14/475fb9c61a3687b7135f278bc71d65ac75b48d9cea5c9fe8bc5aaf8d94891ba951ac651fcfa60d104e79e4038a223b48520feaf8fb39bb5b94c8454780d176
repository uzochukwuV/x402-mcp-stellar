...
exports.TrezorModule = exports.TREZOR_ID = void 0;
...
class TrezorModule {
    constructor(params) {
        ...
        Object.defineProperty(this, "moduleType", {
            ...
            configurable: true,
            writable: true,
            value: mod_js_3.ModuleType.HW_WALLET
        });
        Object.defineProperty(this, "productId", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: exports.TREZOR_ID
        });
    ...
    async runChecks() {
        if (!this._isAvailable) {
            throw (0, utils_js_1.parseError)(new Error("Trezor connection has not been started yet."));
        }
    }
    async getAddress(opts) {
        await this.runChecks();
        try {
            let mnemonicPathValue = opts?.path || mod_js_2.mnemonicPath.value;
            if (!mnemonicPathValue) {
                mnemonicPathValue = mod_js_2.mnemonicPath.value;
            }
            if (!mnemonicPathValue)
    ...
    async getAddresses(page = 0) {
        ...
        mod_js_2.hardwareWalletPaths.value = results;
        return results;
    }
    async signTransaction(xdr, opts) {
        ...
        if (opts?.path) {
            ...
            const paths = mod_js_2.hardwareWalletPaths.value;
            const target = paths.find((p) => p.publicKey === opts.address);
            if (!target)
                throw (0, utils_js_1.parseError)(new Error("This address has not been loaded from this device"));
            ...
            mnemonicPathValue = mod_js_2.mnemonicPath.value;
            if (!mnemonicPathValue) {
                throw (0, utils_js_1.parseError)(new Error("There is no path available, please call the `getAddress` method first."));
            }
            const result = await TrezorConnect.stellarGetAddress({ path: mnemonicPathValue, showOnTrezor: false });
            if (!result.success) {
                throw new Error(result.payload.error);
        ...
        const network = opts?.networkPassphrase || mod_js_2.selectedNetwork.value;
        if (!network)
