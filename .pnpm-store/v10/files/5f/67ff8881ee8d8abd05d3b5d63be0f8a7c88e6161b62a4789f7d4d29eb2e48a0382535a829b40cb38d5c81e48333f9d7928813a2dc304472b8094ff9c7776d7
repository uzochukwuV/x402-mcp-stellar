...
import { AssetUtil, ChainController, ConnectorController, ModalController } from '@reown/appkit-controllers';
...
describe('W3mPayLoadingView', () => {
    let subscribeKeyCallbacks = new Map();
    let subscribeChainPropCallback = null;
    ...
    beforeEach(() => {
        ...
        vi.spyOn(PayController, 'subscribeKey').mockImplementation((key, callback) => {
            subscribeKeyCallbacks.set(key, callback);
            return vi.fn();
        });
        ...
        vi.spyOn(ConnectorController, 'subscribeKey').mockImplementation((key, callback) => {
            subscribeKeyCallbacks.set(`connector-${key}`, callback);
            return vi.fn();
        });
    ...
    test('should render success state with checkmark icon', async () => {
        ...
        const element = await fixture(html `<w3m-pay-loading-view></w3m-pay-loading-view>`);
        await elementUpdated(element);
        const successIcon = element.shadowRoot?.querySelector('.token-image.success wui-icon[name="checkmark"]');
    ...
    test('should display payment lifecycle steps', async () => {
        ...
        const sendingStep = texts.find(el => el.textContent?.trim() === 'Sending asset to the recipient address');
    ...
    test('should show checkmarks for completed steps when status is pending', async () => {
        ...
        const successBoxes = Array.from(stepBoxes || []).filter(box => box.classList.contains('success'));
        expect(successBoxes.length).toBe(1);
        const checkmarks = Array.from(element.shadowRoot?.querySelectorAll('wui-icon[name="checkmark"]') || []);
    ...
    test('should show close icons for all steps when status is failure', async () => {
        ...
        const element = await fixture(html `<w3m-pay-loading-view></w3m-pay-loading-view>`);
        await elementUpdated(element);
        const closeIcons = Array.from(element.shadowRoot?.querySelectorAll('wui-icon[name="close"]') || []);
    ...
    test('should update UI when account data changes', async () => {
        ...
        if (subscribeChainPropCallback) {
            subscribeChainPropCallback({
                currentTab: 0,
                caipAddress: 'eip155:1:0x9876543210987654321098765432109876543210',
